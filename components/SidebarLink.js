...
export default function SidebarLink(props) {
    ...
    const sizeStylesMap = {
        small: `py-2xs px-sm text-xs`,
        medium: `py-xs px-sm text-sm`,
    ...
    //'juno-current-color-bg opacity-100' : 'juno-current-color-hover-bg opacity-80'} 
    let innerClasses = `${isActive ? 'opacity-100' : 'opacity-70 hover:opacity-100'}
        transition-all relative flex flex-row items-center justify-between cursor-default duration-150 
        w-full ${sizeStyles} ${cornerStyles} ${borderStyles} ${gapStyles}`
    ...
    const LeftIconComponent = leftIcon !== 'none' ? <Icon icon={leftIcon?.toLowerCase()} className={`flex-shrink-0 scale-90  ${iconWidth}`} defaultIconSet={defaultIconSet} /> : null;
